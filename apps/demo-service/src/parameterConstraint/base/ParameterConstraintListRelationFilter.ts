/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ParameterConstraintWhereInput } from "./ParameterConstraintWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class ParameterConstraintListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => ParameterConstraintWhereInput,
  })
  @ValidateNested()
  @Type(() => ParameterConstraintWhereInput)
  @IsOptional()
  @Field(() => ParameterConstraintWhereInput, {
    nullable: true,
  })
  every?: ParameterConstraintWhereInput;

  @ApiProperty({
    required: false,
    type: () => ParameterConstraintWhereInput,
  })
  @ValidateNested()
  @Type(() => ParameterConstraintWhereInput)
  @IsOptional()
  @Field(() => ParameterConstraintWhereInput, {
    nullable: true,
  })
  some?: ParameterConstraintWhereInput;

  @ApiProperty({
    required: false,
    type: () => ParameterConstraintWhereInput,
  })
  @ValidateNested()
  @Type(() => ParameterConstraintWhereInput)
  @IsOptional()
  @Field(() => ParameterConstraintWhereInput, {
    nullable: true,
  })
  none?: ParameterConstraintWhereInput;
}
export { ParameterConstraintListRelationFilter as ParameterConstraintListRelationFilter };
